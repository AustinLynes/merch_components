...
export const Footer = (props) => {
    ...
    return (
        <FOOTER>
            ...
            {
                links && links.map((link, i) => (<Link key={i + '_lf'} onChangeCallback={OnChangeCallback} style={link.style} name={link.name} edit={editables.filter((ed) => {console.log(ed); ed.id !== i}).edit} />))
            }
        </FOOTER>
    )
}
