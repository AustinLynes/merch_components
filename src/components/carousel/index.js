...
const Carousel = (props) => {
    const { images, style, count, incrementCB, decrementCB, ...rest } = props;
    ...
    if (images && images.length > 0) {
        return (
            <CAROUSEL data-testid='wrapper' style={style && style.wrapperStyle}>
                <ICON data-testid={'icon decrement'} draggable={false} id='left' style={style && style.leftIconStyle} onClick={decrementCB} />
                {
                    images.map((image, i) => (
                        i === count && <IMAGE draggable={false} id={`${i}_im`} data-testid='image' key={`${i}_im`} style={style && style.imageStyle} src={image} />)
                    )
                }
                <ICON data-testid={'icon increment'} draggable={false} style={style && style.rightIconStyle} onClick={incrementCB} />
            </CAROUSEL>
        )
    } else {
        return (
            <PLACEHOLDER_CAROSEL data-testid='wrapper' style={style && style.placeholder_wrapperStyle}>
                <ICON data-testid={'icon'} style={style && style.leftIconStyle} draggable={false} onClick={decrementCB} />
                ...
                <ICON data-testid={'icon'} style={style && style.rightIconStyle} draggable={false} onClick={incrementCB} />
            </PLACEHOLDER_CAROSEL>
        )
    }
}
